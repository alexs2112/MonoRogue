...
namespace MonoRogue {
    public class CreatureScreen : BorderedScreen {
        ...
        public override void Draw(GameTime gameTime, SpriteBatch spriteBatch, MouseHandler mouse) {
            ...
            int x = 48;
            int y = 32;
            spriteBatch.DrawString(Font.Size24.Get(), Creature.Name, new Vector2(x - 16, y), Color.White);
            ...
            spriteBatch.DrawString(Font.Size14.Get(), $"Damage: {damage.Min}-{damage.Max}", new Vector2(x, y), Color.White);
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Range: {Creature.GetRange()}", new Vector2(x, y), Color.White);
            y += 48;

            int parry = Creature.GetParryChance();
            if (parry > 0) {
                spriteBatch.DrawString(Font.Size14.Get(), $"Parry Chance: {parry}%", new Vector2(x, y), Color.White);
                y += 48;
            }
            int crit = Creature.GetCritChance();
            if (crit > 0) {
                spriteBatch.DrawString(Font.Size14.Get(), $"Critical Chance: {crit}%", new Vector2(x, y), Color.White);
                y += 48;
            }
        ...
        private int DrawHealthAndArmor(SpriteBatch spriteBatch, int x, int y) {
            spriteBatch.DrawString(Font.Size14.Get(), "Health: ", new Vector2(x, y), Color.White);
            MainInterface.DrawHearts(spriteBatch, Creature.MaxHP, Creature.HP, x + 142, y - 8, Color.Red);

            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), "Defense: ", new Vector2(x, y), Color.White);
            if (Creature.GetDefense().Max == 0) {
                spriteBatch.DrawString(Font.Size14.Get(), "None", new Vector2(x + 160, y), Color.Gray);
            } else {
                MainInterface.DrawHearts(spriteBatch, Creature.GetDefense().Max, Creature.GetDefense().Current, x + 160, y - 8, Color.LightSkyBlue);
            }

            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Block: {Creature.GetBlock()}", new Vector2(x, y), Creature.GetBlock() > 0 ? Color.White : Color.Gray);

            return y + 48;
        }

        private int DrawDelay(SpriteBatch spriteBatch, int x, int y) {
            spriteBatch.DrawString(Font.Size14.Get(), "Move Delay: ", new Vector2(x, y), Color.White);

            Color c = Color.LightGray;
            int moveDelay = Creature.GetMovementDelay();
            if (moveDelay < 8) { c = Color.LightSeaGreen; }
            else if (moveDelay > 12) { c = Color.LightSalmon; }
            spriteBatch.DrawString(Font.Size14.Get(), moveDelay.ToString(), new Vector2(x + 18 * 12, y), c);

            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), "Attack Delay: ", new Vector2(x, y), Color.White);

            c = Color.LightGray;
            int attackDelay = Creature.GetAttackDelay();
            if (attackDelay < 8) { c = Color.LightSeaGreen; }
            else if (attackDelay > 12) { c = Color.LightSalmon; }
            spriteBatch.DrawString(Font.Size14.Get(), attackDelay.ToString(), new Vector2(x + 18 * 14, y), c);
            
            return y + 48;
        }

        private int DrawEquipment(SpriteBatch spriteBatch, int x, int y) {
            spriteBatch.DrawString(Font.Size16.Get(), "Equipment", new Vector2(x - 16, y), Color.White);
            y += 32;
            if (Creature.Weapon == null) {
                spriteBatch.DrawString(Font.Size14.Get(), "Unarmed", new Vector2(x + 16, y), Color.Gray);
                y += 32;
            } else {
                y = DrawWeapon(spriteBatch, x + 48, y, Creature.Weapon);
            }
            if (Creature.Armor == null) {
                spriteBatch.DrawString(Font.Size14.Get(), "No Armor", new Vector2(x + 16, y), Color.Gray);
                y += 48;
            } else {
        ...
        private int DrawWeapon(SpriteBatch spriteBatch, int x, int y, Weapon w) {
            spriteBatch.DrawString(Font.Size16.Get(), w.Name, new Vector2(x - 32, y), Color.White);
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Damage: {w.Damage.Min}-{w.Damage.Max}", new Vector2(x, y), Color.White);
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Range: {w.Range}", new Vector2(x, y), Color.White);
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Attack Delay: {w.Delay}", new Vector2(x, y), Color.White);
            return y + 32;
        }
        private int DrawArmor(SpriteBatch spriteBatch, int x, int y, Armor a) {
            spriteBatch.DrawString(Font.Size16.Get(), a.Name, new Vector2(x - 32, y), Color.White);
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Defense:", new Vector2(x, y), Color.White);
            MainInterface.DrawHearts(spriteBatch, a.MaxDefense, a.Defense, x + 160, y - 8, Color.LightSkyBlue);
            if (a.Block > 0) {
                y += 32;
                spriteBatch.DrawString(Font.Size14.Get(), $"Block: {a.Block}", new Vector2(x, y), Color.White);
            }
            y += 32;
            spriteBatch.DrawString(Font.Size14.Get(), $"Weight: {a.Weight}", new Vector2(x, y), Color.White);
            return y + 32;
        }
        ...
        private int DrawAbilities(SpriteBatch spriteBatch, int x, int y) {
            ...
            spriteBatch.DrawString(Font.Size16.Get(), "Abilities", new Vector2(x - 16, y), Color.White);
            y += 32;

            foreach (string s in AbilityText) {
                spriteBatch.DrawString(Font.Size14.Get(), s, new Vector2(x+16, y), Color.White);
                y += 32;
            }
